...
- iOS/Android can't launch here (no simulator/Expo Go), but you can validate the native build end-to-end without a device by bundling: `npx expo export --platform ios` (or `--platform android`) must complete with no resolution/transform errors. The dev server also serves native bundles (e.g. `curl "http://localhost:8081/node_modules/expo-router/entry.bundle?platform=ios&dev=true"` should return HTTP 200). `metro.config.js`'s `unstable_enablePackageExports` applies to all platforms and does not break native resolution.
- `npx expo-doctor` reports two pre-existing, non-blocking warnings (legacy top-level `expo.splash` key in `app.json`, and `eas-cli` listed as a dependency); these do not affect iOS/Android bundling or runtime.
